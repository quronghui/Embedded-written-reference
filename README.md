...
3. [C函数strcpy，memcpy，sprintf, memset区别](/8.1Dji大疆/C函数使用注意.md)
   + 实现memcpy()函数
   + 实现strcpy()函数
...
4. Linux把名称中含有test内容的进程找出来杀死?](/shell/Linux把名称中含有test内容的进程找出来杀死.md)
5. [Linux读取某个文件的某一行或者某几行, 使用sed](/shell/Linux读取某个文件的某一行或者某几行.md)
6. [Linux 磁盘整体情况](/shell/Linux磁盘整体情况.md)
7. [grep邮件地址正则表达式的匹配书写](https://luckywater.top/2019/04/25/Regular-Expression/)
