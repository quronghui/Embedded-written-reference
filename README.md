...
5. [linux 内存管理 malloc 分配用户空间？](/OS/linux内存管理.md)
6. [内存管理中mmap的原理？](/OS/内存管理中mmap的原理.md)
7. [内存管理中kmalloc和vmalloc分配内核空间](/OS/内存管理中kmalloc和vmalloc的区别.md)
...
6. [Linux的**用户态与内核态 **?](https://luckywater.top/2019/08/02/linux用户和内核/)
   1. Linux的用户态与内核态 定义?
...
3. 未全部完成 --*[Linux内核中常用的两个宏定义?](/8.1Dji大疆/linux内核中常用的两个宏定义.md)
4. [Linux内核的五个模块描述](/OS/Linux内核的五个模块描述.md)
5. [Linux的具体的命令, 文件操作的5个命令?]
6. [Makefile文件的编写?]
...
1. [OS多任务嵌入式系统中，将寄存器REGn的指定位反转](/8.1Dji大疆/OS嵌入式系统指定位反转.md)
2. [嵌入式系统中，要求程序员去访问某特定的内存位置的特点](/OS/访问固定的内存位置.md)
3. [如何写C嵌入式的死循环](/OS/如何写C嵌入式的死循环/md)
4. [C语言**编译执行**的四个阶段?](/8.1Dji大疆/C语言编译执行的四个阶段.md)
5. [简述**C函数中参数**传递问题](/8.1Dji大疆/简述C函数中参数传递问题.md)
6. [C函数参数传递的三种方式, 传值, 指针, 引用?](/C/指针和引用的区别.md)
7. [C : 当char类型变量进行赋值时, 超过其范围时怎么处理?](https://github.com/quronghui/DataStructAndAlogrithmCode/blob/master/CompanyWrite/1_Dji/charConvertint.c)
8. [C: 有符号的变量(负数)和无符号的变量(正数)相加](/8.1Dji大疆/C有符号的变量和无符号的变量相加.md)
9. [C: 关键字用途的描述](/8.1Dji大疆/C关键词的描述.md)
10. [C简述static关键字对于工程模块化的作用?](/C/C简述static关键字对于工程模块化的作用.md)
11. [如何理解嵌入式的volatile变量？](/OS/嵌入式的volatile变量.md)
12. [C中全局变量和局部变量可以重名吗?](/8.24HIK/C中全局变量和局部变量可以重名.md)
13. [C 宏定义的用法](/8.24HIK/C宏使用.md)
14. [C 不同变量的生命周期?](/C/C变量的生命周期.md)
15. [C 运算符的优先级](/C/C 运算符的优先级.md)
16. [预处理器标识#error的目的是什么](/OS/error的目的是什么.md)
17. [用变量a给出下面的定义，包含指针？](/C/用变量a给出下面的定义.md)
18. [C什么是左值，什么是右值]
19. [C: 如何实现一个数的四舍五入?]
